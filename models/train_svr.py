"""
SVR Model Training Script for Building Energy Consumption Prediction
Trains a Support Vector Regression model with hyperparameter tuning and saves results
"""
...
def prepare_data_for_svr(X_train, X_test, y_train, y_test, use_subset=True, subset_size=5000):
    ...
    Args:
        ...
        use_subset (bool): Whether to use a subset for training (SVR is computationally expensive)
...
def plot_support_vectors_analysis(model, X_train, save_path="outputs/charts/support_vectors_analysis_svr.png"):
    """
    ...
    ax1.set_title('Training Samples vs Support Vectors')
    ...
    if n_features >= 2:
        ...
        ax2.set_title('Support Vectors Distribution (First 2 Features)')
        ax2.legend()
        ax2.grid(alpha=0.3)
    else:
...
def plot_kernel_analysis(model, save_path="outputs/charts/kernel_analysis_svr.png"):
    """
...
"""
    ...
    ax.set_ylim(0, 1)
    ax.axis('off')
    ax.set_title('SVR Model Parameters and Statistics', fontsize=14, fontweight='bold')
...
def save_model_and_predictions(model, y_test_pred, scaler=None, 
                             model_path="outputs/model_svr.pkl", 
                             predictions_path="outputs/predictions_svr.csv",
                             scaler_path="outputs/scaler_svr.pkl"):
    """
    ...
    if scaler is not None:
        ...
        print(f"SVR scaler saved to: {scaler_path}")
...
def main():
    ...
    """
    print("="*60)
    print("SVR MODEL TRAINING PIPELINE")
    print("="*60)
    ...
    # Plot support vectors analysis
    plot_support_vectors_analysis(best_model, X_train_svr)
    
    # Plot kernel analysis
    plot_kernel_analysis(best_model)
    
    # Save model and predictions
    save_model_and_predictions(best_model, best_metrics['test_predictions'], scaler)
    
    print(f"\n" + "="*60)
    print("TRAINING PIPELINE COMPLETED SUCCESSFULLY!")
    print("="*60)
    print("Files saved:")
    print("  - outputs/model_svr.pkl")
    print("  - outputs/predictions_svr.csv") 
    print("  - outputs/scaler_svr.pkl")
    print("  - outputs/charts/support_vectors_analysis_svr.png")
    print("  - outputs/charts/kernel_analysis_svr.png")
