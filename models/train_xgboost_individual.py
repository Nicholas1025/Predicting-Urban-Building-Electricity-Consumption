"""
XGBoost Model Training Script for Individual Dataset Analysis
Trains an XGBoost regressor with hyperparameter tuning for specific dataset
"""
...
def get_output_paths(dataset_name):
    ...
    return {
        'data_dir': base_dir,
        'model_path': f"{base_dir}/model_xgb.pkl",
        'pred_path': f"{base_dir}/predictions_xgb.csv",
        'chart_path': f"{base_dir}/charts/feature_importance_xgb.png"
    }
...
def load_preprocessed_data(dataset_name=None):
    ...
    Args:
        dataset_name (str): Name of the dataset (e.g., 'seattle_2015')
...
def plot_feature_importance(model, feature_names, save_path, dataset_name=""):
    ...
    title = f'Top 20 Most Important Features - XGBoost Model'
    if dataset_name:
...
def main(dataset_name=None):
    ...
    print("="*60)
    print(f"XGBOOST MODEL TRAINING PIPELINE{dataset_info}")
    print("="*60)
